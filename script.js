...
class button {
    constructor(rusValue ,engValue ,eventcode ,shiftEngValue, boardKey){
    ...
    HtmlButton(buttonline) {
        ...
        var eventcodecopy = this.eventcode;
        var keycopy = this.boardKey;

        window.addEventListener("keydown",function (key) {
            if(key.which === keycopy && key.which !== 16 && key.which !== 17 && key.which !== 18){
                button.style.opacity = "0.5"       
            }
            if(eventcodecopy === key.code){
                button.style.opacity = "0.5"
            }
...
keyBoard.id = "board"
    ...
    new button("х","[","Ё","~",219).HtmlButton(buttonline)
    ...
    var buttonline = document.createElement('div')
    new button("SHIFT","SHIFT","ShiftLeft","SHIFT",16).HtmlButton(buttonline)
    ...
    new button("PAUSE","PAUSE","ShiftRight","PAUSE",16).HtmlButton(buttonline)

    keyBoard.appendChild(buttonline)
    buttonline.className = "buttonline"

    var buttonline = document.createElement('div')
    new button("CTRL","CTRL","ControlLeft","CTRL",17).HtmlButton(buttonline)
   // new button("win","win","win","win",91).HtmlButton(buttonline)
    new button("ALT","ALT","AltLeft","ALT",18).HtmlButton(buttonline)
    new button("","","","",32).HtmlButton(buttonline)
    new button("ALT","ALT","AltRight","ALT",18).HtmlButton(buttonline)
    new button("CTRL","CTRL","ControlRight","CTRL",17).HtmlButton(buttonline)
